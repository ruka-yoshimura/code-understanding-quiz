...
本プロジェクトは学習用環境での稼働を終了しており、現在はソースコードのみ公開しています。
## アプリの使い方
...
- **AIのハルシネーションを防ぐ**:
  AIが間違ったクイズや解説を作らないよう、AI自身に「自分の作った問題が正しいか」を二重チェックさせる仕組みや、公式のドキュメントを参考にしながら問題を解くような仕組みを取り入れ、情報の正確さを高める
- **悪意のある操作（プロンプトインジェクション）への対策**:
  「AIに命令を無視させる」ような特殊なコードが入力されても、AIが惑わされずに正しくクイズを作成できるように、セキュリティ対策を強化する
...
- **ランキング機能**: 獲得した経験値（XP）でユーザー同士が競い合えるランキングを実装し、学習を継続する楽しさを高める
