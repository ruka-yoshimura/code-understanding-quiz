...
class QuizGeneratorService
  ...
  def prompt
    <<~TEXT
      あなたはプログラミングの先生です。以下のコード（言語はコード内容から自動的に判断してください）に関する「3択クイズ」を1問作成してください。
  ...
  def difficulty_prompt
    ...
    when 11..20
      "【中級者向け】
      配列や連想配列（マップ、辞書など）の操作、繰り返し処理、関数の定義と引数の受け渡しなど、データの流れを追う必要がある問題にしてください。"
    when 21..30
      "【上級者向け】
      クラスやオブジェクトの利用、例外処理、名前空間、モジュール化など、複数の要素が組み合わさった「少し複雑な構造」の問題にしてください。エッジケース（特殊な条件下での挙動）も歓迎します。"
    else
      "【エキスパート向け】
      メモリ管理、スコープの詳細、高階関数、クロージャ、並行処理、またはその言語特有の高度な機能（メタプログラミング、デコレータ、ポインタ、ジェネリクスなど）に関する、深い洞察が必要な問題にしてください。"
    end
  end
