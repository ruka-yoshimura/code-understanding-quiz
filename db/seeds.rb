...
# 1. デバッグ用ユーザーの作成
Rails.logger.debug 'Creating debug users...'
...
user_a = User.create!(
  ...
  xp: 0,
  daily_streak: 1,
  last_answered_date: Time.zone.today
)
...
Rails.logger.debug 'Debug users created:'
Rails.logger.debug ' - User A: Lv 1 (Beginner)'
Rails.logger.debug ' - User B: Lv 25 (Seeker)'
Rails.logger.debug ' - User C: Lv 49 (Pro)'

# 2. 「こだわり」のサンプルクイズ（投稿 + クイズ）の作成
Rails.logger.debug 'Creating curated posts and quizes...'
...
Rails.logger.debug 'Generating activity history...'
...
# ゲストユーザー用にもサンプルを用意
Rails.logger.debug 'Creating curated posts for guest user...'
